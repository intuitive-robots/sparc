...
def convert_frames_to_sam2(
    ...
    """Convert the video frames to the format expected by SAM2."""
    ...
    images = [torch.tensor(frame) for frame in frames]
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def init_state(
        ...
        if isinstance(video_path, str):
            ...
            # Assume video_path is a list of numpy arrays
            images, video_height, video_width = convert_frames_to_sam2(
                frames=video_path,
                image_size=self.image_size,
                offload_video_to_cpu=offload_video_to_cpu,
                compute_device=compute_device,
    ...
    def _run_single_frame_inference(
        ...
        mark_step_begin = getattr(self, "_mark_cudagraph_step_begin", None)
...
class SAM2VideoPredictorVOS(SAM2VideoPredictor):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.compile_mode = os.environ.get("SAM2_COMPILE_MODE", "max-autotune")
        self._compile_all_components()

    def _mark_cudagraph_step_begin(self):
        mark_step_begin = getattr(torch.compiler, "cudagraph_mark_step_begin", None)
        if mark_step_begin is not None:
            mark_step_begin()
        
    def _compile_all_components(self):
        print(f"Compiling all components for VOS setting. First time may be very slow. Compile mode: {self.compile_mode}")
