...
sys.path.append(user_home +"/code/dataset_creator/detectors/sam2")
...
class SAM2ImagePredictor:
    ...
    def set_image(
        ...
        if isinstance(image, torch.Tensor):
            # SAM2 transforms expect Numpy (H,W,C). 
            # If tensor is on GPU, move to CPU. If (C,H,W), permute it.
    ...
    def predict(
        ...
        masks_np = masks.squeeze(0).float().detach().cpu().numpy().copy()
        iou_predictions_np = iou_predictions.squeeze(0).float().detach().cpu().numpy().copy()
        low_res_masks_np = low_res_masks.squeeze(0).float().detach().cpu().numpy().copy()
    ...
    def _prep_prompts(
        ...
        if point_coords is not None:
            ...
            if isinstance(point_coords, torch.Tensor):
                point_coords = point_coords.to(dtype=torch.float, device=self.device)
            else:
